...
// this function works...but is doing way too much. Large functions like this tend
// to be brittle and hard to maintain and test a function should do one thing and do it well. Not everything!
async function everything() {
	...
	// This local query won't display, because here in
	// Argentina, the items received in the feature array, are 0.
	let query =
		baseUrl +
	...
	// listHtml is an array, it is necessary to merge it in one single string.
	console.log(`typeof(listHtml) → ${typeof listHtml}`);
	console.log(`Array.isArray(listHtml) → ${Array.isArray(listHtml)}`);
	...
	// attach a listener to watch for a click on the quake. If it sees one then render out the
